    ...
    try {
      const pdfBuffer: Buffer = await new Promise(async (resolve) => {
        ...
        doc.table(tableData, {
          ...
          prepareRow: (row, indexColumn, indexRow, rectRow, rectCell) => {
            const { x, y, width, height } = rectCell;
            console.log('first', { x, y, width, height });
            ...
            if (row[5] === 'working day' && indexColumn === 5) {
              doc.addBackground(rectCell, '#1FEE20', 0.2);
            } else if (row[5] === 'absent' && indexColumn === 5) {
              doc.addBackground(rectCell, '#DD3227', 0.2);
            } else if (row[5] === 'weekend' && indexColumn === 5) {
              doc.addBackground(rectCell, '#0B4AE5', 0.2);
            } else if (row[5] === 'holiday' && indexColumn === 5) {
              doc.addBackground(rectCell, '#E5B917', 0.2);
            } else if (row[5] === 'leave' && indexColumn === 5) {
              doc.addBackground(rectCell, '#8E04EB', 0.2);
            }
    ...
    try {
      const pdfBuffer: Buffer = await new Promise(async (resolve) => {
        ...
        for (let i = 10; i < 1000; i += 20) {
          doc.lineWidth(0.5).moveTo(1000, 150).lineTo(10, 150).stroke();
          doc
            .lineWidth(0.5)
            .moveTo(x, y + i)
            .lineTo(x, y + height + i)
